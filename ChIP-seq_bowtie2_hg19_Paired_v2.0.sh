...
do
    ...
    echo "
	Step3: 使用 bowtie2 对处理后的文件进行比对
	"
    bowtie2 -p 20 --reorder -N 1 --very-sensitive-local -x hg19 --local --no-mixed --no-discordant -X 1000 --phred33 -1 "${file/_1.fq.gz/_1_val_1.fq}" -2 "${file/_1.fq.gz/_2_val_2.fq}" -S "${file/_1.fq.gz/}.sam" 
    ...
    echo "
	Step8: 利用bamcoverage生成可视化文件，参数： --normalizeUsing RPGC --binSize 20 --effectiveGenomeSize 2864785220（hg19） -p max --smoothLength 40  --ignoreDuplicates  --extendReads
	"
    bamCoverage --normalizeUsing RPGC --binSize 20 --effectiveGenomeSize 2864785220 -p max --smoothLength 40  --ignoreDuplicates  --extendReads  -b "${file/_1.fq.gz/_rmdup}.bam"  -o ./BigWig/"${file/_1.fq.gz/}.bw" 

	echo "
	Step9: 开始callpeak
	"
	
    macs2 callpeak -t "${file/_1.fq.gz/_rmdup}.bam"  -f BAMPE -g hs --outdir ./MACS2/"${file/_1.fq.gz/}"/ -n "${file/_1.fq.gz/-SPMR}" -q 0.01 --cutoff-analysis
